...
import frc.robot.util.LoggedStateIO
...
class State(
    ...
    fun push(
        ...
        nextCage: BargePosition = this.nextCage,
    ...
    companion object {
        ...
        fun new(
            ...
            nextCage: BargePosition = BargePosition.NONE,
    ...
    override fun toString(): String {
        when (task) {
            AutoTask.IDLE -> return "Idle"
            AutoTask.TO_CORAL_STATION -> {
                return when (nextStation) {
                    PathfindingFactories.CoralStationSide.LEFT -> "Pathfind to Left Coral Station"
                    PathfindingFactories.CoralStationSide.RIGHT -> "Pathfind to Right Coral Station"
                    PathfindingFactories.CoralStationSide.NONE -> "You shouldn't ever see this, something has gone very wrong!"
                }
            }

            AutoTask.TO_REEF -> {
                return "Pathfind to Reef $nextReef, $nextState ($coralStatus, $algaeStatus)"
            }

            AutoTask.TO_BARGE -> {
                return "Pathfind to Barge $nextBarge, $nextState ($coralStatus, $algaeStatus)"
            }
            ...
            AutoTask.WAITING -> return "Waiting for input..."
...
open class StateIO {
    ...
    var nextCage: BargePosition = BargePosition.NONE
