...
public class Climber extends SubsystemBase {
  ...
  public Command moveClimberToIntakePosition() {
    return runOnce(() -> winchIO.setWinchBrakeMode(NeutralModeValue.Coast)).andThen(
        run(() -> pivotIO.setPivotPosition(0.0))).withName("Move Climber to Intake Position");
  }

  public Command moveClimberToCageCatchPosition() {
    return runOnce(() -> winchIO.setWinchBrakeMode(NeutralModeValue.Coast)).andThen(
        run(() -> pivotIO.setPivotPosition(ClimberConstants.getPivotCageCatchPosition()))).withName("Move Climber to Cage Catch Position");
  }

  public Command climb() {
    Elastic.sendNotification(new Notification(NotificationLevel.INFO, "Info", "Godspeed, soldier."));
    return runOnce(() -> {
      pivotIO.setWinchBrakeMode(NeutralModeValue.Coast);
      winchIO.setWinchBrakeMode(NeutralModeValue.Brake);
    }).andThen(
        run(() -> winchIO.setWinchOpenLoop(0.75)).until(() -> pivotIO.getPivotPosition() < -0.16667)
    ).withName("Climb");
  }
